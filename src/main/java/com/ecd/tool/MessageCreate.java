...
public class MessageCreate {
	...
	public static MimeMessage createMimeMessage(String vText, Session session, String receiveMail) throws Exception {
		...
		// 2. From: 发件人
		message.setFrom(new InternetAddress(myEmailAccount, "环保信访管理系统", "UTF-8"));
		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "系统系统用户", "UTF-8"));
		// 4. Subject: 邮件主题
		message.setSubject("环保局消息通知", "UTF-8");
		...
		message.setContent(vText, "text/html;charset=UTF-8");
		System.out.println("发送的内容是： " + vText);
		// 6. 设置发件时间
